...
multiAsset :: forall era. (Scriptic era, HasTokens era) => Proof era -> MultiAsset (Crypto era)
multiAsset pf = forge @era 1 (always 2 pf)

validatingTxWithMintOut :: forall era. (HasTokens era, EraTxOut era, Scriptic era, Value era ~ MaryValue (Crypto era)) => Proof era -> TxOut era
